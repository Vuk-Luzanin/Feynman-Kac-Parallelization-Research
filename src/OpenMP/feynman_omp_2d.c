...
#define MAX_INSIDE  NI*NJ
static grid_point_t inside_points[MAX_INSIDE];

double feynman_7(const double a, const double b, const double h, const double stepsz, const int N) 
{
  int seed = 123456789;
  double err = 0.0;
  ...
  // cuvanje svih tacaka koje se nalaze unutar elipse, koje ce zapravo biti obradjene
  for (int i = 1; i <= NI; i++)
  {
    for (int j = 1; j <= NJ; j++ )
    {
      ...
      inside_points[n_inside].j = j;
      inside_points[n_inside].x = x;
      inside_points[n_inside].y = y;

      n_inside++;
    }
  }

  for (int n = 0; n < n_inside; n++)
  {
    int i = inside_points[n].i;
    int j = inside_points[n].j;
    double x1 = inside_points[n].x;
    double x2 = inside_points[n].y;

    w_exact[i][j] = exp(pow(x1 / a, 2) + pow(x2 / b, 2) - 1.0);
